...
## 2026-08-08
- **Omnical-ändringar tappades bort — två separata buggar, båda fixade** (commit `627936e`). Symptom: en redigering gjord i Omnical kunde blinka till i canvas och sedan försvinna, ibland med Souls gamla text tillbakaskriven över anteckningen på disk. Riktningen Soul→Omnical var opåverkad.
  1. **Adoptionens merge-baslinje var fel.** När en pending-fil adopterades (`omnicalSync.ts`, adoptionsloopen i `runSyncLocked`) sattes `omnicalLink.bodyHash` till hashen av *filens nuvarande* body, medan `node.content` medvetet behölls som Souls text. `planSharedMerge` läste då Souls orörda text som "ändrad" och remote som oförändrad → Soul vann body och skrev sin gamla text över filen, i samma `runSync` som adopterade den. Baslinjen är nu `pending.bodyHash` (texten Soul faktiskt publicerade), vilket ger en verklig trevägsmerge. Tags/meta behåller remote-hasharna med flit: `publishShareRequests` skriver rå body utan frontmatter, så de fanns aldrig på disk och Soul äger dem fortfarande.
  2. **Kort-fil-synken och Omnical-synken kunde klobbra varandra.** `runCardFileScan`, `saveFile` och `runSync` gjorde alla `getState()` → disk-I/O → `setState` av en hel nodes-Map, utan gemensamt lås — `withCardFileLock` satt *inuti* `scanCardFiles`/`syncCardFiles` och skyddade bara I/O:t, inte snapshotet. Båda syncarna triggas dessutom av `window.focus` med 250 ms debounce. Den som landade sist återställde den andres merge. `withCardFileLock` exporteras nu från `cardFileSync.ts` och hålls av alla tre över hela cykeln; de inre låsen är borta. Notera: `scanCardFiles`/`syncCardFiles` låser alltså inte längre själva — nya anropare **måste** ta låset.
- **Verifiering:** regressionstest för (1) i `tests/omnicalSync.test.ts` ("lets an Omnical edit made while the card is still pending survive adoption") — failar mot den gamla logiken med `expected 'Souls text' to be 'Ändrad i Omnical'`. (2) har ingen testtäckning: racet går inte att framkalla med den minnesbaserade FS-mocken. `npm run sanity` grön (68 tester).
- **Medvetet orört:** auto-importen i `omnicalSync.ts` (varje Omnical-anteckning med `omnicalId` blir ett kort i Omnical-sessionen) — avsett beteende, inte en bugg, men det är den som gör att kort "dyker upp i den andra appen".
- **Kända kvarvarande smågrejer** (fanns före ovanstående): en pending som Omnical både döper om och redigerar innan adoption kan fastna på "Väntar på Omnical" permanent (både path- och fingerprint-matchningen missar); `disconnectOmnicalFolder` under pågående sync kan få statusen överskriven till `synced`.
- **Att komma ihåg vid testning:** `enableOmnicalSharedNotes` är `false` som standard och bor i localStorage **per origin**, och Omnical-mappkopplingen i IndexedDB likaså — flaggan måste slås på separat på varje origin (canvas.ximon.se vs localhost).
